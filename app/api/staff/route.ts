...
export async function GET() {
  try {
      ...
      return NextResponse.json(
        {
          ...
          details:
            "NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY is not set in Vercel environment variables.",
        },
        { status: 400 },
      );
    }
    ...
    console.log(
      `[v0] Fetch staff list. Service Role: ${serviceRoleKey ? "YES" : "NO"}`,
    );
    ...
    ({ data: staff, error } = await supabase
      .from("users")
      .select(
        "*, staff_photos(photo_url), latitude, longitude, last_location_update",
      )
      .neq("role", "admin")
    ...
    if (error && error.message?.includes("does not exist")) {
      console.warn(
        "[v0] Location columns missing, falling back to query without them:",
        error.message,
      );
      ({ data: staff, error } = await supabase
        .from("users")
    ...
    const formattedStaff = staff.map((user: any) => {
      ...
      return {
        ...
        photo_url:
          user.photo_url ||
          user.staff_photos?.[0]?.photo_url ||
          `https://ui-avatars.com/api/?name=${encodeURIComponent(user.full_name || "S")}&background=random&color=fff`,
...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      "[v0] API Request received. Keys:",
      Array.from(formData.keys()),
    );
    if (photoFile) {
      console.log(
        `[v0] photoFile detected: name=${photoFile.name}, size=${photoFile.size}, type=${photoFile.type}`,
      );
    } else {
      ...
      return NextResponse.json(
        {
          error: "Missing Configuration",
          details:
            "NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY is not set in environment variables.",
        },
        { status: 400 },
      );
    }
    ...
    console.log(
      `[v0] Supabase client init. Service Role: ${serviceRoleKey ? "YES" : "NO (using Anon Key - RLS will apply)"}`,
    );
    const supabase = createClient(supabaseUrl, serviceRoleKey || supabaseKey);
    ...
    // 1. Calculate DB Role
    const dbRole = role.toLowerCase().includes("manager")
      ? "manager"
    ...
    if (photoFile && photoFile.size > 0) {
      console.log(
        `[v0] Processing photo: ${photoFile.name}, size: ${photoFile.size} bytes`,
      );
    ...
    const { error: dbError } = await supabase.from("users").upsert({
      ...
      hire_date:
        (formData.get("hire_date") as string) ||
