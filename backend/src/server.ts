...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (!authData.user) {
      return res
        .status(401)
        .json({ error: "User not found after successful auth" });
    }
  ...
  async (req: any, res) => {
    try {
      ...
      if (files.photo && files.photo[0]) {
        ...
        const { data: photoData, error: photoUploadError } =
...
app.get("/api/staff", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error while fetching staff" });
  }
});
...
app.get("/api/staff/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error while fetching staff member" });
  }
});
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      return res.status(403).json({
        error:
          "Permission denied or user not found. Real database change failed.",
      });
    }
    ...
    res
      .status(500)
      .json({ error: "Internal server error while updating status" });
  }
});
...
app.delete("/api/staff/:id", async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to remove staff profile from database" });
    }

    if (!data || data.length === 0) {
      return res.status(403).json({
        error:
          "Permission denied or user already deleted. Database delete blocked.",
      });
    }
    ...
    res
      .status(500)
      .json({ error: "Internal server error while deleting staff" });
  }
});
